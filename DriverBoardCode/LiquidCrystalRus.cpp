...
void LiquidCrystalRus::beginWithSPI(void)
{
	...
	delayMicroseconds(25000);
	  //For SPI  bit0=not used, bit1=RS, bit2=RW, bit3=Enable, bits4-7 = DB4-7
	  //We start in 4bit mode, try to set 4 bit mode
	  //_bitString |= ((1<<5) | (1<<4));// This data to take in datasheet
	  //pulseEnable();
	  mySend(0x03);
	  //doing 2
	  ...
	  mySend(0x03);
	  //pulseEnable();
	  delayMicroseconds(40000); // wait min 40.1ms
     //doing 3
	  mySend(0x03);
	  //pulseEnable();
      delayMicroseconds(40000);
...
void LiquidCrystalRus::mySend(uint8_t value)
{
	 bitWrite(_bitString, _rs_pin, 0); //set RS to mode
	 //spiSendOut();
